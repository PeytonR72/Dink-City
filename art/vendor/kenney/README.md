...
CC0 models by Kenney (www.kenney.nl), used by `art/scripts/map.py` for the Dink City map's buildings and a few plants. (Its roads are built in `map.py`, in the colors of Kenney's City Kit (Roads), so each end of the avenue can turn.) Only the files the map uses are kept. Each pack's `License.txt` is its original license.
